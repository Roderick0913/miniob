...
{
  LOG_TRACE("PlainCommunicator_write_result start");
  ...
  LOG_TRACE("PlainCommunicator_write_result finish");
  return rc;
}

RC PlainCommunicator::write_result_internal(SessionEvent *event, bool &need_disconnect)
{
  LOG_TRACE("PlainCommunicator_write_result_internal start");
  RC rc = RC::SUCCESS;
  need_disconnect = true;
  ...
  LOG_TRACE("TupleCell write to Buff start");
  ...
  LOG_TRACE("TupleCell write to Buff finish");

  LOG_TRACE("add endl start");
  ...
  while (RC::SUCCESS == (rc = sql_result->next_tuple(tuple))) {
    ...
    LOG_TRACE("cell_num = %d" , cell_num);
    for (int i = 0; i < cell_num; i++) {
      LOG_TRACE("%d" , cell_num);
      if (i != 0) {
        const char *delim = " | ";
        rc = writer_->writen(delim, strlen(delim));
        if (OB_FAIL(rc)) {
  ...
  LOG_TRACE("PlainCommunicator_write_result_internal finish");
  return rc;
}