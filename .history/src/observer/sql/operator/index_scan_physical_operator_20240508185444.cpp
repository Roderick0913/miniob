...
{
  LOG_TRACE("IndexScanPhysicalOperator->next start");
  RID rid;
  RC rc = RC::SUCCESS;
  ...
  LOG_TRACE("IndexScanPhysicalOperator->next finish");
  return rc;
}
...
{
  LOG_TRACE("filter start");
  RC rc = RC::SUCCESS;
  Value value;
  for (std::unique_ptr<Expression> &expr : predicates_) {
    rc = expr->get_value(tuple, value);
    LOG_WARN("value=%d attrtype=%d" , value.get_int() , value.attr_type());
    if (rc != RC::SUCCESS) {
      return rc;
    }
