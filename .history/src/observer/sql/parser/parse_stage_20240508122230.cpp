...
RC ParseStage::handle_request(SQLStageEvent *sql_event)
{
  ...
  {
    LOG_WARN("condition left_value:type=%d , data=%d, right_value:type=%d ,data=%s",
    selection_node_ptr.conditions[i].right_value.attr_type() ,selection_node_ptr.conditions[i].right_value.get_string(),
    selection_node_ptr.conditions[i].left_value.attr_type() ,selection_node_ptr.conditions[i].left_value.get_string()
    );
  }
