...
// 这部分给我的感觉是单纯是赋值，将值赋值给value，
// 主要考虑的是值的类型，不太需要考虑，值赋值给value后应该为什么类型
// 简单来说其实就是初始化。
...
Value::Value(AttrType attr_type , int val){s}

// 这个是已知value是什么类型，将该类型进行赋值
void Value::set_data(char *data, int length)
{
  switch (attr_type_) {
    ...
    case DATES: {
      num_value_.int_value_ = *(int *)data;
      LOG_TRACE("int_value=%d",num_value_.int_value_);
      length_               = length;
    } break;
    default: {
