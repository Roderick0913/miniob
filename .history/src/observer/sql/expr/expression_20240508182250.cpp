...
RC ComparisonExpr::get_value(const Tuple &tuple, Value &value) const
{
  ...
  LOG_TRACE("left_value type=%d data=%d" , left_value.attr_type() , left_value.get_int());
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to get value of left expression. rc=%s", strrc(rc));
    return rc;
  }
  rc = right_->get_value(tuple, right_value);
  LOG_TRACE("right_value type=%d data=%d" , right_value.attr_type() , right_value.get_int());
  if (rc != RC::SUCCESS) {
